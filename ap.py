...
# def show_image_only(path):
#     epd = epd13in3E.EPD()
#     try:
#         epd.Init()
#         epd.Clear()  # clear once to reduce ghosting
...
#         # If image is swapped vs panel buffer, rotate
#         if img.size == (h, w):
...
# def fetch_image_to_file(url: str, dest_dir: str = "/tmp", max_bytes: int = 25 * 1024 * 1024) -> str:
#     os.makedirs(dest_dir, exist_ok=True)

#     def _guess_ext(content_type: str, url_: str) -> str:
#         mapping = {
#             "image/jpeg": ".jpg",
#             "image/png": ".png",
#             "image/gif": ".gif",
...
#             "image/tiff": ".tif",
#         }
#         if content_type and content_type.lower().split(";")[0] in mapping:
#             return mapping[content_type.lower().split(";")[0]]
#         ext = os.path.splitext(urlparse(url_).path)[1].lower()
...
#             with requests.get(url, stream=True, timeout=(8, 30)) as r:
#                 r.raise_for_status()
#                 ext = _guess_ext(r.headers.get("Content-Type", ""), url)
#                 fpath = os.path.join(dest_dir, f"epaper_{int(time.time())}{ext}")
#                 total = 0
...
# def show_ap_qr_on_epaper(ssid="EpaperConfig", portal_host="192.168.4.1:8080"):
#     """
#     Attractive AP instruction screen:
#       - Centered title + underline
#       - Three left-aligned steps with comfortable line spacing
#       - Large centered QR in the lower half
#       - Crisp 1-bit output for e-paper
#     Saves to /tmp/epaper_qr.png and displays via show_image_only(path).
#     Returns True/False from show_image_only.
#     """
#     from PIL import Image, ImageDraw, ImageFont

#     # --- Get panel size (avoid extra 90° path in your loader) ---
#     try:
#         epd = epd13in3E.EPD()
#         W, H = epd.width, epd.height
#     except Exception:
#         W, H = 1600, 1200  # fallback

#     # --- Scale by screen size (baseline: 1200 short side) ---
#     base = min(W, H)
#     k = base / 1200.0

#     # Font sizes tuned for 13.4"
#     title_fs = max(56, int(72 * k))
#     text_fs  = max(36, int(44 * k))
#     step_fs  = max(34, int(38 * k))
#     small_fs = max(28, int(36 * k))

#     # --- Fonts (fallback-safe) ---
#     try:
#         font_path = "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"
#         title_font = ImageFont.truetype(font_path, title_fs)
#         text_font  = ImageFont.truetype(font_path, text_fs)
#         step_font  = ImageFont.truetype(font_path, step_fs)
#         small_font = ImageFont.truetype(font_path, small_fs)
#     except Exception:
#         title_font = text_font = step_font = small_font = ImageFont.load_default()

#     # --- Helpers ---
#     def text_sz(draw, text, font):
#         try:
...
#     # --- Canvas (grayscale → 1-bit at end) ---
#     img = Image.new("L", (W, H), 255)
#     draw = ImageDraw.Draw(img)

#     # --- QR payload (Wi-Fi auto-join for OPEN) ---
#     qr_img = None
#     try:
#         import qrcode  # pip install "qrcode[pil]"
#         payload = f"WIFI:T:nopass;S:{ssid};P:;H:false;;"
#         qr = qrcode.QRCode(version=None, box_size=10, border=2)
#         qr.add_data(payload); qr.make(fit=True)
#         qr_img = qr.make_image(fill_color="black", back_color="white").convert("L")
#         qr_side = int(min(W, H) * 0.45)  # bigger QR (~45% of short side)
#         qr_img = qr_img.resize((qr_side, qr_side), Image.NEAREST)
#     except Exception as e:
#         print(f"QR generation unavailable: {e}")

#     # --- Layout metrics ---
#     margin_top   = int(0.08 * H)
#     side_margin  = int(0.08 * W)               # wider side margin for left-aligned text
#     underline_w  = max(4, int(5 * k))
#     border_w     = max(4, int(5 * k))
#     gap_small    = max(8, int(10 * k))         # small gap
#     gap_lines    = max(12, int(18 * k))        # line spacing for steps (“little bit space”)

#     # --- Title (centered) with subtle bold/shadow, then underline ---
#     title = "Wi-Fi Configuration Required"
#     tw, th = text_sz(draw, title, title_font)
#     tx = (W - tw) // 2
#     ty = margin_top
#     for dx, dy in ((-1,0),(1,0),(0,-1),(0,1)):
#         draw.text((tx+dx, ty+dy), title, font=title_font, fill=0)
...
#     u_x1 = (W - u_w) // 2
#     u_x2 = u_x1 + u_w
#     draw.line((u_x1, y, u_x2, y), fill=0, width=underline_w)
#     y += gap_lines

#     # --- Three-step block (LEFT-ALIGNED) ---
#     x_left = side_margin  # left alignment anchor
#     # Header line (left-aligned)
#     header = "Follow these steps to connect:"
#     hw, hh = text_sz(draw, header, text_font)
#     draw.text((x_left, y), header, font=text_font, fill=0)
#     y += hh + (gap_small // 2)

#     steps = [
#         f"1) Connect to Wi-Fi:  {ssid} (OPEN NETWORK)",
#         f"2) Open your browser: http://{portal_host}",
#         "3) Select your network and enter password",
#     ]
#     for line in steps:
#         lw, lh = text_sz(draw, line, step_font)
#         draw.text((x_left, y), line, font=step_font, fill=0)
#         y += lh + gap_lines  # comfortable spacing

#     # --- Large QR centered in LOWER HALF for balance ---
#     label_lh = 0
#     if qr_img:
#         qr_x = (W - qr_img.width) // 2
#         # target area ~ lower 40% of the screen
#         target_center_y = int(H * 0.52)
#         qr_y = max(y + gap_small, target_center_y - qr_img.height // 2)
#         img.paste(qr_img, (qr_x, qr_y))

#         label = "Scan to Connect"
#         lw, label_lh = text_sz(draw, label, small_font)
#         draw.text((qr_x + (qr_img.width - lw)//2, qr_y + qr_img.height + gap_small),
#                   label, font=small_font, fill=0)
#         block_bottom = qr_y + qr_img.height + gap_small + label_lh
#     else:
#         block_bottom = y

#     # --- Decorative outer border to frame content ---
#     pad = int(0.03 * base)
#     draw.rectangle(
#         (side_margin - pad, margin_top - pad, W - side_margin + pad, block_bottom + pad),
#         outline=0, width=border_w
#     )

#     # --- Save & display ---
#     out_path = "/tmp/epaper_qr.png"
#     img.convert("1").save(out_path)
#     return show_image_only(out_path)

# # ---------- end Image helpers ----------

#===========================================================================================================================================================================================

# ---------- aws cetificate creaction  ----------

def aws_certs_present(certs_dir=CERTS_DIR, required=REQUIRED_CERTS) -> bool:
    """Return True if all required cert files exist in the certs_dir."""
    try:
        return all((certs_dir / name).exists() for name in required)
    except Exception:
        return False

def ensure_aws_iot_certs_once():
    """
    If certs are missing, run aws/register_and_init_shadows.py exactly ONCE
    in a subprocess so sys.exit() inside that script can't kill this process.
    """
    global AWS_INIT_DONE
    if AWS_INIT_DONE:
        return

    if aws_certs_present():
        print("✓ AWS IoT certificates already present.")
        AWS_INIT_DONE = True
        return

    try:
        base_dir = os.path.dirname(os.path.realpath(__file__))
        reg_script = os.path.join(base_dir, "aws", "register_and_init_shadows.py")
        if not os.path.isfile(reg_script):
            print(f"✗ Registration script not found at: {reg_script}")
            AWS_INIT_DONE = True
            return

        print("⚙️  AWS IoT certificates missing — running registration script once …")
        # Run as a separate process to sandbox sys.exit() from that script
        cp = subprocess.run([sys.executable, reg_script], check=False)
        rc = cp.returncode
        if rc != 0:
            print(f"⚠️  Registration script exited with code {rc}")

        # Re-check after running
        if aws_certs_present():
            print("✓ AWS IoT certificates created successfully.")
        else:
            print("✗ Registration finished but certificates are still missing.")

    except Exception as e:
        print(f"✗ Error while initializing AWS IoT certificates: {e}")

    # Never run this again in this process
    AWS_INIT_DONE = True

# ---------- aws cetificate creaction end  ----------
